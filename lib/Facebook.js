...
function Facebookbot(configuration) {
            ...
            request.post('https://graph.facebook.com/me/messages?access_token=' + configuration.access_token,
                function(err, res, body) {
                    ...
                    try {
                        ...
                        var original_json = JSONbig.parse(body);
                        var json = JSONbig.stringify(original_json)
    ...
    facebook_botkit.createWebhookEndpoints = function(webserver, bot) {
        ...
        webserver.post('/facebook/receive', function(req, res) {
            ...
            var obj = JSONbig.parse(req.body);
            console.log(obj)
            if (obj.entry) {
                for (var e = 0; e < obj.entry.length; e++) {
                    for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                        var facebook_message = obj.entry[e].messaging[m];
                        var fbSenderId = facebook_message.sender.id.toString();
    ...
    facebook_botkit.setupWebserver = function(port, cb) {
        ...
        facebook_botkit.webserver.use(bodyParser.text({ type: 'application/json' }));
